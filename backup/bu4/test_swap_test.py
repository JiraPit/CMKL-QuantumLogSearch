...
def test_swap_test_identical():
    ...
    def circuit():
        ...
        # Apply swap test
        swap_test.build_circuit(state1_wires=[0, 1], state2_wires=[2, 3], ancilla_wire=4)
    ...
    assert zeros/1000 > 0.9, "Expected probability of |0⟩ to be close to 1 for identical states"
...
def test_swap_test_orthogonal():
    ...
    assert 0.4 < zeros/1000 < 0.6, "Expected probability of |0⟩ to be close to 0.5 for orthogonal states"
...
def test_swap_test_partial_overlap():
    ...
    for theta in angles:
        ...
        assert abs(prob_0 - expected_prob_0) < 0.1, f"Probability {prob_0} too far from expected {expected_prob_0}"

...
class GroverSearch:
    ...
    def _oracle(self):
        ...
        For swap test: |0⟩ means high similarity, |1⟩ means low similarity
        """
        ...
        # The result is stored in the ancilla qubit
        self.swap_test.build_circuit(self.data_wires, self.omega_wires, self.ancilla_wire)
        
        # In swap test, |0⟩ means "similar" and |1⟩ means "different"
        # But for Grover's search, we need to mark the states we want to find
        # So we need to flip the meaning of the ancilla
        qml.PauliX(wires=self.ancilla_wire)
        
        # Now perform controlled phase flip on index register
        # This marks index states that correspond to vectors similar to omega
        for wire in self.index_wires:
            qml.ctrl(qml.PauliZ, control=self.ancilla_wire, control_values=1)(wires=wire)
        
        # Uncompute the ancilla to avoid entanglement
        # First, undo the X gate
        qml.PauliX(wires=self.ancilla_wire)
        
        # Then, undo the swap test
        self.swap_test.build_circuit(self.data_wires, self.omega_wires, self.ancilla_wire)
